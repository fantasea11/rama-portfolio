...
const app = express();
app.use(cors({
  origin: '*', // Allow all origins for local development and cross-device testing
...
const migrateData = async () => {
  try {
    ...
    if (userCount > 0) {
      ...
      if (existingSettings) {
        const updatedSettings = await Models.Settings.findOneAndUpdate(
          {}, 
          { 
            $set: {
              instagram: existingSettings.instagram || '',
              instagramName: existingSettings.instagramName || '',
              linkedin: existingSettings.linkedin || '',
              linkedinName: existingSettings.linkedinName || '',
              github: existingSettings.github || '',
              githubName: existingSettings.githubName || '',
              discord: existingSettings.discord || '',
              discordName: existingSettings.discordName || '',
              tiktok: existingSettings.tiktok || '',
              tiktokName: existingSettings.tiktokName || ''
            }
          },
          { new: true }
        );
        console.log('Settings updated with social media fields and display names.');
      } else {
